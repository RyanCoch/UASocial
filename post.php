...
function parse_links($text)
{
   ...
   if($lines > 150 || $text==" ##clear"){
		...
		$fh = fopen("log.php", 'w');
		fwrite($fh,"<?php \$a = session_id();");
		fwrite($fh,"if(empty(\$a)) session_start();");
		fwrite($fh,'if(!isset($_SESSION[\'uid\'])){');
		fwrite($fh,"echo '<META HTTP-EQUIV=\"Refresh\" Content=\"0; URL=index.php\">'; exit; } ?>");
		for($i = 0; $i < 50; $i++){
		fwrite($fh, $line[$i]);
		}
		fclose($fh);
	...
	if ($text==" ##clear") exit();
		...
		$line = "<div class=\"msgln sex".$chat_color."\"><span class=\"span1\">";
		$line .= "<a href=\"profile.php?id=$uid\"><img class=\"userface\" src=\"pictures/$picture"."30.jpg\" /></a></span>";
